...
## 2026-06-17 · Session 1 (continued)

### Entry #51 · Deeper dashboard (admin charts + user progress + Available Tasks) + /plans centering + installable PWA
- **User prompt (Banglish):** "temon kono change pai nai, available task shoho baki card gulo redesign hoy nei, dashboard a admin valo bujhbe knw, user dashboard kemon hobe, sob shundor koro; 2nd — /plans e plan card left side a, ami chai middle theke (1 ta → middle, 2/3 ta → middle theke 2 side a adjust); sob page mobile/tablet responsive PWA style."
- **Plan**: approved plan at `C:\Users\dmmah\.claude\plans\announcements-total-2ta-active-sunny-honey.md` (overwrote Entry #50's). Workflow: 3 Explore agents (DoableTasksPreview + analytics catalogue + recharts pattern / plans grid / PWA+responsive baseline) → direct reads (plans-view, tasks-view DoableTaskRow, analytics getUserDashboardStats + ReportFilters, apple-icon, MILESTONE_BONUSES, proxy) → AskUserQuestion (3 decisions: **add admin charts** + **installable PWA no-offline** + **focused responsive**) → ExitPlanMode → approved.
- **Part 1 — /plans centering ([components/shared/plans-view.tsx](components/shared/plans-view.tsx)):** the plan grid + packages grid used `grid … lg:grid-cols-3 max-w-5xl mx-auto`, which centers the container but **left-aligns cards when <3**. Swapped both (and the loading skeleton) to `flex flex-wrap justify-center gap-… max-w-5xl mx-auto`; each plan `<Card>` now `w-full sm:w-72 lg:w-80`, each package card `w-full sm:w-72`. Result: 1 plan = centered, 2/3 = balanced from the middle, 4+ = wraps centered. `-top-3` ribbons + popular `lg:scale-[1.02]` unaffected (flex-wrap handles `overflow-visible`).
- **Part 2 — deeper dashboard:**
  - **Available Tasks ([tasks-view.tsx](components/shared/tasks-view.tsx) `DoableTasksPreview`):** the one card never redesigned — added a gradient header icon tile (`ListTodo`, primary→accent) matching Entry #50's other cards; flattened `DoableTaskRow`'s outer `<Card>` (`shadow-none border-border/60 hover:border-primary/30`) so rows read as a list, not cards-in-a-card. Accept/submit/view logic + mobile layout untouched.
  - **Admin overview charts (NEW) — [dashboard-content.tsx](components/shared/dashboard-content.tsx) + [page.tsx](app/(dashboard)/dashboard/page.tsx):** page.tsx now fetches `getCompletionTrend({})` + `getAssignmentStatusDistribution({})` for admins (empty filter → last-30-days; both staff-gated). New `AdminCharts` component renders a recharts **stacked AreaChart** (completed/submitted/rejected) + a **donut PieChart** with a legend (status color map mirroring reports-view's `COLORS`), in a `lg:grid-cols-2` motion-staggered row; returns `null` / "No data yet" when empty. recharts `PieChart` aliased to `RePieChart` to avoid the lucide `PieChart` icon clash.
  - **User progress widget (NEW):** extended `getUserDashboardStats` ([lib/actions/analytics.ts](lib/actions/analytics.ts)) to also return `tasksCompleted` + `streak` (widened the existing `profiles` select to `total_points, tasks_completed, current_streak` — no extra query). New `UserProgress` card shows a milestone progress bar toward the next `MILESTONE_BONUSES` threshold (10/50/100/500/1000 → +bonus pts), a `Flame` streak pill, and "N more tasks" copy. Inserted after the stat grid (admin → charts, user → progress).
- **Part 3 — installable PWA (no offline):** NEW [app/manifest.ts](app/manifest.ts) (`MetadataRoute.Manifest`: name/short_name/description, `start_url /dashboard`, `display standalone`, `theme_color #9333ea`, 192/512 + maskable icons); NEW `app/icon-192.png/route.tsx` + `app/icon-512.png/route.tsx` (ImageResponse route handlers reusing the apple-icon gradient + lightning mark, `force-static`). **proxy fix:** added `webmanifest` to `STATIC_ASSET_RE` in [proxy.ts](proxy.ts) — `/manifest.webmanifest` was being auth-redirected (307); browsers fetch it unauthenticated, so it must pass the static bypass. iOS Add-to-Home already worked via the existing `apple-icon.tsx`. No service worker.
- **Part 4 — responsive (focused):** charts use `ResponsiveContainer` (fluid) + stack to 1-col on mobile; hero chips already `flex-wrap`; plans flex-wrap centers at every width; Available Tasks keeps its mobile layout. Shell/nav untouched (already responsive).
- **Verified:** `pnpm exec tsc --noEmit` clean (source; only regeneratable `.next/dev/types/*` cache errors, not my code); `pnpm exec eslint <8 touched files + proxy.ts>` clean; dev server (port 3001) `GET /` 200, `/dashboard` 307, `/plans` 307, **`/manifest.webmanifest` 200** (was 307 before the proxy fix — confirmed body has name + icons), `/icon-192.png` 200, `/icon-512.png` 200.
- **Files touched:** `components/shared/plans-view.tsx`, `components/shared/tasks-view.tsx`, `components/shared/dashboard-content.tsx`, `app/(dashboard)/dashboard/page.tsx`, `lib/actions/analytics.ts`, `proxy.ts`; NEW `app/manifest.ts`, `app/icon-192.png/route.tsx`, `app/icon-512.png/route.tsx`. No migration.
- **State:** Complete. /plans cards center from the middle; admin dashboard gains completion-trend + status-distribution charts; user dashboard gains a milestone/streak progress card; Available Tasks card now matches the new design language; the app is installable (manifest + icons + standalone).
- **Visual test path:** (admin) /dashboard → charts row (area + donut, last 30 days) under stat cards; (user) /dashboard → "Your Progress" milestone bar + streak pill, redesigned Available Tasks; /plans with 1 plan → centered, with 2/3 → centered group; Chrome → Install prompt available, opens standalone; resize 375/768/1024/1440 holds.
- **Next step:** None on its own (browser confirmation by user).

---

## 2026-06-17 · Session 1

### Entry #50 · Announcements infinite-loop fix + dashboard modern redesign (gradient hero + motion)
- **User prompt (Banglish):** "Announcements total 2ta active ache, 1ta shes hoile arekta ashe change hoy, then atke jay, ami caitesi loop thakle valo hoy, jei koytai thakuk na knw 1,2,3 jokhn dekhano shes hobe then abr 1,2,3 ei vabe jolte thakbe, new jodi abr ad kora hoy tokhn 1,2,3,4 then abr 1,2,3,4 … dashboard ta old style a ache eita new UI update kore shundor kore deo."
- **Plan**: approved plan at `C:\Users\dmmah\.claude\plans\announcements-total-2ta-active-sunny-honey.md`. Workflow: 2 Explore agents (notice rotation state machine + dashboard structure) → direct reads of the 5 key files → AskUserQuestion (2 decisions: **Full modern redesign** + **Yes, subtle framer-motion**) → ExitPlanMode → approved.
- **BUG (root cause — confirmed):** the announcements spotlight ([components/shared/notice-board.tsx](components/shared/notice-board.tsx)) auto-rotate interval binds with deps `[items.length, paused]` and calls `slideTo(next)`. But `slideTo` is re-created each render and its guard `if (animating || idx === activeIdx) return` read the **closed-over** `activeIdx`/`animating` frozen at effect-bind time (`activeIdx = 0`). So rotation advanced 1→2→…→N fine, but the modulo wrap back to index 0 hit `0 === 0` (stale) and bailed — it stuck on the last notice and never looped. Exactly matched "shows them, then atke jay."
- **FIX (Part 1):** `slideTo` now guards on **live refs** (`activeIdxRef.current` + new `animatingRef`) and sets `activeIdxRef.current = idx` immediately in the slide timeout so back-to-back interval ticks read fresh state. Now it wraps to 0 and loops forever for any count; a new notice (60s refetch / invalidation changes `items.length`) re-binds the interval and extends the loop (1,2,3 → 1,2,3,4 …). Moved `slideTo`'s declaration **above** the auto-rotate effect (it referenced it before declaration → react-hooks error once the stale `eslint-disable` was removed). Replaced the planned setState-in-effect clamp with a **render-time derived `safeIdx`** (`activeIdx < items.length ? activeIdx : 0`) used by the counter / prev-next / dots / progress key / `current` — avoids the new `react-hooks/set-state-in-effect` rule and still self-heals after a notice is deleted (interval also self-heals via modulo).
- **REDESIGN (Part 2 — full + motion):**
  - **[components/shared/stat-card.tsx](components/shared/stat-card.tsx):** flat `bg-{accent}/10` icon tiles → **gradient tiles** (`ACCENT_GRADIENT` map: primary=primary→accent, warning=amber→orange, success=emerald→green, accent=pink→rose) in `text-white` with per-accent glow + `group-hover:scale-105`; card hover upgraded to `hover:shadow-lg` + per-accent `ACCENT_HOVER` tint. (StatCard is consumed ONLY by the dashboard — grep-verified — so the visual change is scoped.)
  - **[components/shared/dashboard-content.tsx](components/shared/dashboard-content.tsx):** rewritten with `framer-motion` (`^12.38.0`, already in stack). New local **gradient welcome hero** (`bg-linear-to-br from-primary via-primary to-accent`, white text, blurred decorative blobs, Sparkles eyebrow) replacing the shared `<PageHeader>` on the dashboard only; right side shows 3 highlight chips from the **already-fetched** stats (admin: Total Tasks/Pending/Active Users; user: My Tasks/Points(`formatPoints`)/Rank — no new query). Stat cards now built from a `stats[]` array + `.map`, each wrapped in `motion.div` with a staggered container (`staggerChildren 0.06`, item `{opacity:0,y:8}→{opacity:1,y:0}`, 0.3s). Activity + Top-Performer cards wrapped in the same motion stagger; each `CardHeader` got a gradient icon tile (Activity / Award) and rows got `hover:bg-muted/40 rounded-lg px-2 -mx-2` tint. User Quick-Stats tiles got gradient icon bubbles. All admin/user branches, `PerfAvatar`, medal ranks, status meta, `enable_notice_board` + subscription gating preserved.
- **Connected features preserved:** `app/(dashboard)/dashboard/page.tsx` unchanged (existing `Promise.all` stats cover the hero chips); shared `PageHeader` (27 other pages) untouched; NoticeBoard modal + arrows + dots + hover-pause + `useActiveNotices` 60s poll intact; no new analytics query, no migration.
- **Verified:** `pnpm exec tsc --noEmit` clean for source (only regeneratable `.next/dev/types/*` cache artifacts errored — same as Entry #19, not my code); `pnpm exec eslint <3 files>` clean; dev server (port 3001) `GET /` 200, `GET /dashboard` 307 (auth-gated).
- **Files touched:** `components/shared/notice-board.tsx`, `components/shared/stat-card.tsx`, `components/shared/dashboard-content.tsx`.
- **State:** Complete. Announcements now loop continuously regardless of count and extend when a notice is added; dashboard has a gradient hero, gradient stat tiles, and staggered motion entrance.
- **Visual test path (login → /dashboard):** with ≥2 active notices the spotlight auto-rotates and keeps looping (1→2→1→2 …) without sticking; add a 3rd notice → within ~60s loop becomes 1→2→3→1 …; arrows/dots/hover-pause/modal still work. Dashboard shows the purple→pink hero with greeting + highlight chips, gradient stat-card icon tiles with hover lift, and activity/performer cards fading in with a gentle stagger; holds at 375/768/1024/1440px.
- **Next step:** None on its own (browser confirmation by user).
